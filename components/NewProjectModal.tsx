...
export function NewProjectModal({ isOpen, onClose }: NewProjectModalProps) {
  ...
  // État de la sous-modale d'ajout d'image
  ...
  const handleAddImageToGallery = () => {
    ...
    // Reset et fermeture de la popup image
  ...
  return (
    <>
      <div className="fixed inset-0 z-40 flex items-center justify-center p-4">
        ...
        <div className="relative z-50 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl border border-white/10 bg-(--site-ink) p-8 shadow-2xl">
          <button
            onClick={onClose}
          ...
          <h2 className="mb-6 text-2xl font-semibold text-(--site-gold)">Nouveau Projet</h2>
          ...
          <form className="space-y-8" onSubmit={(e) => e.preventDefault()}>
            ...
            <div>
              ...
              <div 
                ...
                className="group relative flex h-48 cursor-pointer overflow-hidden flex-col items-center justify-center rounded-2xl border-2 border-dashed border-white/20 bg-black/20 hover:border-(--site-gold)/50 hover:bg-black/40 transition"
              >
                {coverPreview ? (
                  <img src={coverPreview} alt="Cover preview" className="absolute inset-0 h-full w-full object-cover opacity-60 group-hover:opacity-40 transition" />
                ) : null}
                <div className="text-center relative z-10">
                  ...
                  <p className="text-sm font-medium text-white">{coverPreview ? "Modifier l'image de couverture" : "Cliquez pour ajouter une image de couverture"}</p>
                  <p className="mt-1 text-xs text-(--site-mist)">PNG, JPG ou WEBP (Max 5MB)</p>
                </div>
              </div>
            </div>
            ...
            <div>
              <label className="mb-2 flex items-center justify-between text-sm font-medium text-(--site-sand)">
                ...
                <span className="text-xs text-white/50">{gallery.length} image{gallery.length > 1 ? "s" : ""} sélectionnée{gallery.length > 1 ? "s" : ""}</span>
              </label>
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                {/* Bouton d'ajout d'image galerie */}
                <div 
            ...
            <div className="mt-8 flex justify-end gap-3 pt-6 border-t border-white/10">
              ...
              <button
                disabled
                className="cursor-not-allowed rounded-full bg-(--site-gold) px-6 py-2.5 text-sm font-semibold text-white opacity-50 shadow-lg"
              >
                Sauvegarder
              </button>
            </div>
      ...
      {/* Sous-Pop-up pour l'ajout d'une image spécifique */}
      {isImageModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative z-[70] w-full max-w-md rounded-3xl border border-white/10 bg-[#16202c] p-6 shadow-2xl">
            <h3 className="mb-4 text-xl font-semibold text-white">Créer une Image</h3>
            ...
            <div className="space-y-5">
                ...
                >
                  {newImagePreview ? (
                    ...
                    <div className="text-center">
                      ...
                      <span className="text-sm text-white/70">Parcourir ou Drag & Drop...</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-white/10">
                ...
                <button
                  ...
                  disabled={!newImageFile && !newImagePreview}
